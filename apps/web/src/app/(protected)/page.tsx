...
export default function DashboardPage() {
    ...
    if (isLoading) {
        return (
            <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-gray-50 via-gray-100 to-gray-50">
                <div className="flex items-center gap-3 text-sm text-gray-500 bg-white px-5 py-3 rounded-xl border border-gray-200/80 shadow-md">
    ...
    if (isError || !data) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-50 via-gray-100 to-gray-50 px-4 py-8">
                <div role="alert" className="mx-auto max-w-5xl rounded-xl bg-red-50 p-4 border border-red-200 shadow-sm">
                    <p className="text-sm font-medium text-red-700 flex items-center gap-2">
                        ...
                        Error al conectar con el servidor de stock.
                    </p>
                </div>
            </div>
        );
    ...
    return (
        <div className="relative min-h-screen bg-gradient-to-br from-gray-50 via-gray-100 to-gray-50 px-4 py-8 sm:px-6 lg:px-8 overflow-hidden">
            <div className="absolute top-0 left-1/4 -z-10 h-72 w-72 rounded-full bg-indigo-400/10 blur-3xl" />
            <div className="absolute bottom-10 right-1/4 -z-10 h-72 w-72 rounded-full bg-sky-400/10 blur-3xl" />

            <main className="mx-auto max-w-5xl bg-white/80 backdrop-blur-md rounded-2xl border border-gray-200/80 p-6 sm:p-8 shadow-xl shadow-gray-200/50 space-y-8">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 pb-6">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold tracking-tight text-gray-950">Dashboard</h1>
                        <p className="mt-1 text-sm text-gray-500">Métricas clave y control de stock en tiempo real.</p>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                        <Link href="/categories" className="btn-secondary text-sm font-medium shadow-sm bg-white">
                            Ver categorías
                        </Link>
                ...
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    {/* Productos */}
                    <div className="card flex flex-col justify-between p-5 border-t-4 border-t-slate-500 bg-white shadow-sm">
                        <p className="text-sm font-medium text-gray-500">Productos</p>
                        <p className="mt-3 text-3xl font-bold tracking-tight text-gray-950">{data.total_products}</p>
                    </div>

                    <div className="card flex flex-col justify-between p-5 border-t-4 border-t-indigo-500 bg-white shadow-sm">
                        <p className="text-sm font-medium text-gray-500">Categorías</p>
                        <p className="mt-3 text-3xl font-bold tracking-tight text-gray-950">{data.total_categories}</p>
                    </div>

                    <div className={`card flex flex-col justify-between p-5 border-t-4 bg-white shadow-sm ${
                        data.low_stock_count > 0 ? "border-t-red-500 bg-red-50/10" : "border-t-emerald-500"
                    }`}>
                    ...
                    <div className="card flex flex-col justify-between p-5 border-t-4 border-t-amber-500 bg-white shadow-sm">
                ...
                <div className="space-y-4 pt-2">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-gray-400 font-medium">Actualizado recientemente</span>
                    </div>
                                ...
                                <table className="w-full text-left text-sm text-gray-600">
                                    <thead className="bg-gray-50 text-xs font-semibold uppercase tracking-wider text-gray-500 border-b border-gray-200">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-6 py-4">ID Producto</th>
                                        ...
                                        {data.recent_movements.map((m) => (
                                            <tr key={m.id} className="hover:bg-indigo-50/30 transition-colors">
                                                ...
                                                <td className="whitespace-nowrap px-6 py-4">
                                                    <span className={`inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${
                                                        m.type === "in" 
                                                ...
                                                <td className="whitespace-nowrap px-6 py-4 text-right font-mono font-bold text-gray-900">
                                                    {m.quantity}
                                                </td>
                                            </tr>
                                        ))}
